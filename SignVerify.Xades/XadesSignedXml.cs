...
namespace SignVerify.Xades
{
    ...
    public class XadesSignedXml : System.Security.Cryptography.Xml.SignedXml
    {
        ...
        public DataObject OriginalData
        {
            get
            {
               return  this.GetOriginalDataObject();
            }
            set
            {
        ...
        public virtual bool ValidateAgainstSchema()
        {
            ...
            try
            {
                schemaStream = assembly.GetManifestResourceStream("SignVerify.Xades.xmldsig-core-schema.xsd");
                xmlSchema = XmlSchema.Read(schemaStream, new ValidationEventHandler(this.SchemaValidationHandler));
                schemaSet.Add(xmlSchema);
                schemaStream.Close();


                schemaStream = assembly.GetManifestResourceStream("SignVerify.Xades.XAdES.xsd");
                xmlSchema = XmlSchema.Read(schemaStream, new ValidationEventHandler(this.SchemaValidationHandler));
                schemaSet.Add(xmlSchema);
                schemaStream.Close();
            ...
            using (XmlTextReader txtReader = new XmlTextReader(this.GetXml().OuterXml, XmlNodeType.Element, xmlParserContext))
            {
        ...
        /// Nissim:replace crypto config with cryptoconfig2 for sha256.
        /// </summary>
        public void ComputeSignature(byte[] fileBytes, string fileName)
        {
            this.BuildDigestedReferences(fileBytes, fileName);
            ...
            //TODO: Send the hash and get the signature back !
            if (hash == null)
            {
            ...
            this.HashValue = this.GetC14NDigest(hash, "ds");
            //
        ...
        /// <summary>
        /// Add new function for compute hash without compute signature
        /// the steps are identical with the compute signature function
        /// but the last step will be external
        /// </summary>
        /// <param name="fileBytes">original content to be signed</param>
        /// <param name="fileName">original file name</param>
        /// <returns></returns>
        public byte[] ComputeHash(byte[] fileBytes, string fileName)
        {

            //this.SignedInfo.SignatureMethod = SignatureMethod;
            this.BuildDigestedReferences(fileBytes, fileName);
            ...
            //this.GetC14NDigest(hash);
            this.m_signature.SignatureValue = new byte[256];
            return this.GetC14NDigest(hash, "ds");
        }

        /// <summary>
        /// insert the external computed signature to the file
        /// </summary>
        /// <param name="signatureValue"></param>
        public void InsertSignatureValue(byte[] signatureValue)
        {  
            //AsymmetricAlgorithm key = SigningKey;
        ...
        #region VerifySignature for sha256
        public bool CheckSignature(CryptoExtractedEntry signedEntry)
        {
            if (!CheckSignatureFormat2())
            {
                return false;
            }


            if (!CheckSignedInfo(GetPublicKey()))
            {
            ...
            if (!CheckDigestedReferences(signedEntry))
            {
                return false;
            }
            return true;
        ...
        private bool CheckSignatureFormat2()
        {
            Type SignedXml_Type = typeof(SignedXml);
            MethodInfo SignedXml_CheckSignatureFormat = SignedXml_Type.GetMethod("CheckSignatureFormat");
            if (SignedXml_CheckSignatureFormat != null)
                return (bool)SignedXml_CheckSignatureFormat.Invoke(this, null);
            MethodInfo SignedXml_DefaultSignatureFormatValidator = SignedXml_Type.GetMethod("DefaultSignatureFormatValidator", BindingFlags.NonPublic | BindingFlags.Static);
            if (SignedXml_DefaultSignatureFormatValidator != null)
                return (bool)SignedXml_DefaultSignatureFormatValidator.Invoke(this, new object[] { this });
            FieldInfo SignedXml_m_signatureFormatValidator = SignedXml_Type.GetField("m_signatureFormatValidator", BindingFlags.NonPublic | BindingFlags.Instance);
            if (SignedXml_m_signatureFormatValidator == null)
                return true;
            Func<SignedXml, bool> m_signatureFormatValidator = (Func<SignedXml, bool>)SignedXml_m_signatureFormatValidator.GetValue(this);
            bool formatValid = m_signatureFormatValidator.Invoke(this);
            return formatValid;
        }

        /// <summary>
        /// Copy of System.Security.Cryptography.Xml.SignedXml.CheckDigestedReferences() which will use 
        /// be able to calculate hash from bytes and not from file uri
        /// </summary>
        internal bool CheckDigestedReferences(CryptoExtractedEntry signedEntry)
        {
            ...
            for (int i = 0; i < references.Count; ++i)
            {
                ...
                Type Signature_Type = typeof(Signature);
                PropertyInfo Signature_ReferencedItems = Signature_Type.GetProperty("ReferencedItems", BindingFlags.NonPublic | BindingFlags.Instance);
                byte[] calculatedHash = new byte[0];

                if (signedEntry != null && signedEntry.FileName.Equals(digestedReference.Uri))
                    calculatedHash = CalculateHashValue(signedEntry.DataBytes, digestedReference.DigestMethod);  
                else
                    calculatedHash = (byte[])Reference_CalculateHashValue.Invoke(digestedReference,
                           new object[] { m_containingDocument, Signature_ReferencedItems.GetValue(m_signature, null) });
        ...
        /// <summary>
        /// Copy of System.Security.Cryptography.Xml.SignedXml.CheckSignedInfo() which will use 
        /// crypto config2 that work with sha256 for compute hash
        /// </summary>
        ...
        /// </summary>
        private void BuildDigestedReferences(byte[] fileBytes, string fileName)
        {
            ...
            foreach (Reference reference2 in list2)
            {
                ...
                object m_containingDocument = SignedXml_m_containingDocument.GetValue(this);
                if (Path.GetFileName(fileName).Equals(reference2.Uri))
                {
                    if (Path.GetExtension(fileName).Equals(".xml"))
                    {
                        XmlDocument doc = Common.Utils.XmlHelper.LoadXmlFromBytes(fileBytes);
                        //XmlReader reader=new XmlTextReader(new MemoryStream(fileBytes));
                        //doc.Load(reader);
                        m_containingDocument = (object)doc;
                        ...
                        Reference_m_refTargetType.SetValue(reference2, ReferenceTargetType.Stream);
                        Reference_m_refTarget.SetValue(reference2, new MemoryStream(fileBytes));
                        Reference_UpdateHashValue.Invoke(reference2, new object[] { m_containingDocument, refList });
                    }
                    else
                        reference2.DigestValue = CalculateHashValue(fileBytes, reference2.DigestMethod);
                }
                else
                    Reference_UpdateHashValue.Invoke(reference2, new object[] { m_containingDocument, refList });
        ...
        private byte[] CalculateHashValue(byte[] fileBytes, string p_digestMethod)
        {
            HashAlgorithm m_hashAlgorithm = CryptoConfig.CreateFromName(p_digestMethod) as HashAlgorithm;
            return m_hashAlgorithm.ComputeHash(fileBytes);
        }
        ...
        private System.Security.Cryptography.Xml.DataObject GetOriginalDataObject()
        {
           foreach(Reference currentRef in this.Signature.SignedInfo.References)
           {
               if (currentRef.Type.Equals(SignedXml.XmlDsigNamespaceUrl + "Object"))
               {
                   foreach (System.Security.Cryptography.Xml.DataObject currentObject in this.Signature.ObjectList)
                   {

                       if (currentRef.Uri.Equals("#"+currentObject.Id))
